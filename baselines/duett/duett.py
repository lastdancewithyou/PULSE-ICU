...
class Model(pl.LightningModule):
    def __init__(self, d_static_num, d_time_series_num, d_target, lr=3.e-4, weight_decay=1.e-1, glu=False,
        ...
        metric_kwargs = {"task": "binary"} if d_target == 1 else {"task": "multilabel", "num_labels": d_target}
        self.train_auroc = torchmetrics.AUROC(**metric_kwargs)
        self.val_auroc = torchmetrics.AUROC(**metric_kwargs)
        self.train_ap = torchmetrics.AveragePrecision(**metric_kwargs)
        self.val_ap = torchmetrics.AveragePrecision(**metric_kwargs)
        self.test_auroc = torchmetrics.AUROC(**metric_kwargs)
        self.test_ap = torchmetrics.AveragePrecision(**metric_kwargs)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.pretrain:
            ...
            if self.pretrain_value:
                if self.pretrain_masked_steps > 1:
                    ...
                    loss = self.pretrain_loss(y_hat_value*mask, y*mask)
                self.log('val_next_loss', loss, on_epoch=True, sync_dist=True, rank_zero_only=True, batch_size=batch_size)
            if self.pretrain_presence:
                if self.pretrain_masked_steps > 1:
                    presence_loss = 0
                    for i in range(self.pretrain_masked_steps):
                    ...
                    presence_loss = self.pretrain_presence_loss(y_hat_presence, mask) * self.pretrain_presence_weight
                self.log('val_presence_loss', presence_loss, on_epoch=True, sync_dist=True, rank_zero_only=True, batch_size=batch_size)
                loss += presence_loss
            if self.predict_events:
                event_loss = self.pretrain_loss(y_hat_events*y_events_mask, y_events*y_events_mask)
                self.log('val_event_loss', event_loss, on_epoch=True, sync_dist=True, rank_zero_only=True, batch_size=batch_size)
                loss += event_loss
        else:
            y_hat = self.forward(self.feats_to_input(x, batch_size))
            if self.pos_frac is not None:
        ...
        self.log('val_loss', loss, on_epoch=True, sync_dist=True, prog_bar=True, rank_zero_only=True, batch_size=batch_size)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.pos_frac is not None:
            ...
            loss = self.loss_function(y_hat, y)
        self.log('test_loss', loss, on_epoch=True, sync_dist=True, rank_zero_only=True, batch_size=batch_size)
