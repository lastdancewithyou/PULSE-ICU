...
class LongformerFinetuneforMultiTask(nn.Module):
    def __init__(self, pretrained_model, num_labels, num_binary_tasks=10, num_sofa_tasks=5,
        ...
        effective_head_dropout = (
            0.0
            if getattr(args, "tuning_mode", "finetune") == "linearprobing"
            else classifier_dropout
        )

        binary_config = deepcopy(pretrained_model.config)
        binary_config.num_labels = 1
        binary_config.classifier_dropout = effective_head_dropout
        self.binary_classifiers = nn.ModuleList([
            onelayer_classifier(binary_config) for _ in range(self.num_binary_tasks)
        ])
        
        sofa_config = deepcopy(pretrained_model.config)
        sofa_config.num_labels = args.num_multiclass_labels
        sofa_config.classifier_dropout = effective_head_dropout
        ...
        phenotype_config.classifier_dropout = effective_head_dropout
        ...
        if getattr(args, "task", None) == "phenotype":
            ...
            if args.window == 24:
                ...
                allowed = [1] if getattr(args, "single_task", None) else [1, 4, 5]
            for i, clf in enumerate(self.binary_classifiers):
                if i not in allowed:
                    for p in clf.parameters():
                        p.requires_grad = False
    ...
    def forward(self, input_ids, value_ids, unit_ids, time_ids,
        ...
        if getattr(self.args, "tuning_mode", "finetune") == "linearprobing":
            # Strict linear probing: deterministic, parameter-free mean pooling.
            mask_f = bool_mask.float().unsqueeze(-1)
            summed = (last_hidden_state * mask_f).sum(dim=1)
            counts = mask_f.sum(dim=1).clamp(min=1.0)
            cls_output = summed / counts
        else:
            # Original downstream design for full finetuning.
        ...
        if getattr(self.args, "window", None) == 0:
            ...
            if self.args.window == 24:
                allowed = [1, 4, 5, 9]
            elif self.args.window == 48:
                allowed = [1] if getattr(self.args, "single_task", None) else [1, 4, 5]
            binary_logits = torch.cat([
                self.binary_classifiers[i](cls_output).unsqueeze(-1) for i in allowed
            ], dim=-1)
            sofa_logits = None
...
class LongformerFinetuneforMultiTask_lora(nn.Module):
    ...
    def forward(
        ...
        # 4) Pooling
        # finetune: learned attention pooling (original downstream design)
        # linearprobing: parameter-free mean pooling so only linear heads train
        if getattr(self.args, "tuning_mode", "finetune") == "linearprobing":
            # Parameter-free pooling for strict linear probing.
            mask_f = bool_mask.float().unsqueeze(-1)
            summed = (last_hidden_state * mask_f).sum(dim=1)
            counts = mask_f.sum(dim=1).clamp(min=1.0)
            cls_output = summed / counts
...
class LongformerFinetuneforPhenotype(nn.Module):
    def __init__(self, pretrained_model, num_labels, classifier_dropout=0.1, freeze_pretrained=True,
        ...
        phenotype_config.num_labels = 25
        if getattr(args, "tuning_mode", "finetune") == "linearprobing":
            phenotype_config.classifier_dropout = 0.0
            self.phenotype_classifier = onelayer_classifier(phenotype_config)
        else:
            phenotype_config.classifier_dropout = classifier_dropout
