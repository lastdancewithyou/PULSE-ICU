...
class EHR_Longformer_Dataset(Dataset):
    ...
    def __init__(
        ...
        vocab_size,
        block_size=17,
        max_length=4093,   # 4093 -> 3000 in finetuning
        use_itemid=True,
        mask_token=1,
        mode="pretrain",
        mask_mode="mlm",
    ...
    def _build_finetune_labels(self, data):
        ...
        if self.selected_data == "P12":
            task_label = torch.tensor(
                [self._require_label(data, "mortality_inhospital")],
                dtype=torch.int64,
            )
            return task_label, None
    ...
    def __getitem__(self, idx):
        ...
        padded_ehr[:seq_len] = ehr
        padded_unit[:seq_len] = unit
        padded_value[:seq_len] = value
        padded_offset[:seq_len] = offset
        padded_token_type[:seq_len] = token_type
